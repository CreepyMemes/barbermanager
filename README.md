...
- The deployed will be available at: [http://manageBarber.s1lentCommit.duckdns.org](http://manageBarber.s1lentCommit.duckdns.org)
...
sudo ln -s /home/dietpi/projects/ManageBarber/Implementazione/nginx/nginx.conf /etc/nginx/sites-available/manageBarber.conf

# Link to sites-enabled
sudo ln -s /etc/nginx/sites-available/manageBarber.conf /etc/nginx/sites-enabled/
```
...
```bash
cd ~/projects/ManageBarber && git pull
```

### 2. Rebuild and run production containers

```bash
cd ~/projects/ManageBarber/Implementazione && docker compose -f docker-compose.prod.yml up -d --build
```
...
```bash
cd ~/projects/ManageBarber/Implementazione && docker compose -f docker-compose.prod.yml exec -it backend sh
```
...
```bash
sudo rm /etc/nginx/sites-enabled/manageBarber.conf
```

### To stop containers

```bash
cd ~/projects/ManageBarber/Implementazione && docker compose -f docker-compose.prod.yml down
```
